...
beforeEach(() => {
	...
	// Identity by default — no reaction bookkeeping on the leaf.
...
describe('POST /avatar/prepare — where a comparison may park', () => {
	...
	it('parks over a reply whose leaf is only its reaction bookkeeping', async () => {
		...
		// — bookkeeping hanging off the reply, not the branch moving on. Read
		// literally, the assistant-only conjunct fails and every reacted-to
		// description answers 409; the avatar flow, asking the assistant to
		// describe itself, is exactly the warm register that draws reactions.
		//
		// Parking rewinds past that tool row and leaves its call unanswered on the
		// branch, which is only sendable because `dropOrphanedToolCalls` removes it
		// at serialization. An earlier attempt at this without that pass produced a
		// payload strict upstreams reject on every later turn.
		leafIs('react-tool');

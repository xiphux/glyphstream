...
<div class="flex h-full flex-col overflow-hidden">
	<header class="shrink-0 px-4 py-3">
		...
		<p class="text-xs text-fg-muted">
			Tools you've granted "always allow" so the assistant skips the approval prompt on subsequent
			calls. Revoke any to return that tool to "ask every time".
		</p>
	</header>
			...
			{#if data.groups.length === 0}
				...
				>
					...
					<p class="mt-2 text-xs">
						When the assistant calls an MCP tool, you can pick "Allow always" in the approval prompt
						to skip the question on future calls.
					</p>
				</div>
			{:else}
					...
					<section class="rounded-lg border border-border bg-surface-panel p-4">
						...
						<ul class="mt-2 flex flex-col gap-1.5">
							{#each group.tools as tool (tool)}
								<li
									class="group flex items-center justify-between gap-3 rounded-md py-2 pl-3 pr-2 text-sm transition hover:bg-surface-sunken/70"
								>

...
export const messages = sqliteTable(
	'messages',
	{
		...
		// The model set this prompt was DISPATCHED to, JSON `CompareSelection[]`
		// (`{modelId, count}[]`). Set on USER rows only, once, at dispatch time.
		//
		// Distinct from `model_used`, which records what an assistant row was
		// actually generated by. Neither the assistant rows nor
		// `conversations.fanout_parent_message_id` can stand in for this:
		// discarding a fan-out result hard-deletes the row (deleteBranch), and
		// the fan-out marker is cleared once the comparison resolves. So a
		// settled fan-out leaves no trace of the models it kicked off, and a
		// retry (which appends a sibling without a new user row) is
		// indistinguishable from a second branch. This column is the durable
		// record, unaffected by both.
		//
		// Null on rows predating the column and on OWUI imports; readers fall
		// back to the reply's `model_used`, then the conversation's model.
		dispatchedModelsJson: text('dispatched_models'),
...
// inlined bodies would exceed a char budget (MEMORY_INLINE_BUDGET_CHARS) the
// store is split into tiers — the highest-scored memories stay inlined in full,
// the rest become a compact `[id] topic` index the model reads back via
// recall_memory. embedding_model records which model produced the vector so a
// model change re-queues the row.
//
// `topic` is a short model-authored label (set as a tool parameter on
// save_memory / update_memory) shown in the index in place of the full body.
// NULL on rows that predate the field; the index falls back to a content
// snippet for those until the phase-3 dreaming pass backfills them.
// `recallCount` / `lastRecalledAt` record recall hits (bumped by recall_memory)
// and feed the tiering score (scoreMemory in server/memory/tiering.ts) that
// decides which memories stay inlined.

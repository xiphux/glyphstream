...
/** Who an assistant bubble presents itself as: the name above it, and the
 *  avatar beside that name (null for every message the conversation's own
 *  model didn't produce, and for a conversation whose model has no avatar). */
...
/**
 * The identity to show above an assistant bubble. Defaults to the
 * conversation's, but when a message was generated by a model OTHER
 ...
 *
 * The avatar rides the SAME attribution test rather than being looked up
 * separately, and that's the point of returning a pair: a preset's avatar is
 * the face of *that preset*, so a sibling some other model generated must not
 * wear it. Resolving the two independently at the call site is exactly how
 * they'd drift — the label would tell the truth while the picture lied.
 * There's no per-base-model avatar to fall back to, so a re-attributed
 * message renders label-only, as every message did before avatars existed.
 */
export function assistantIdentityForMessage(
	m: Pick<ChatMessage, 'role' | 'modelUsed'>,
	conversationModelId: string,
	conversation: AssistantIdentity,

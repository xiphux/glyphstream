...
<script lang="ts">
	...
	let {
		...
		class?: string;
		/** Optional control pinned to the top-right of the thumbnail row (the
		 *  split-attachments toggle). Renders inside the strip so it reads as
		 *  part of the attachment section, not a separate row below it. */
		trailing?: Snippet;
	} = $props();
</script>

{#if attachments.items.length > 0}
	<div class="flex items-start gap-2 border-b border-border pb-2 {className}">
				...
				{:else}
					...
					>
						...
						<button
							...
							title="Remove"
							class="absolute right-0.5 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-media-scrim/80 text-white transition can-hover:opacity-0 group-hover/thumb:opacity-100 hover:bg-media-scrim focus-visible:opacity-100"
						>
		...
		{#if trailing}
			<!-- self-stretch + items-center vertically centers the control against
			     the thumbnail row instead of pinning it to the top. -->
			<div class="flex shrink-0 items-center self-stretch">{@render trailing()}</div>
		{/if}
	</div>
{/if}

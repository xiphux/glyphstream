...
- **Endpoints that share a GPU — shipped, with follow-ons.** `resource_group` on
  `[[endpoints]]` makes two endpoints on one GPU share a gate instead of one
  each, and `release = "llama-cpp-router"` frees the resident model before the
  group's slot is handed to a different member. Four properties of llama.cpp's
  router that its docs don't state, each of which had to be measured and each of
  which is a bug if assumed the other way: the management API is at the server
  root not `/v1`; `/slots?model=` AUTOLOADS a model that isn't loaded (so
  checking whether to unload must never touch an unloaded model); `POST
/models/unload` returns success BEFORE the unload completes (so it has to be
  polled); and a model mid-request is waited for rather than interrupted, since
  it can't be our own traffic and the docs don't define the interrupt. Remaining:

  - _Other backends._ Only `llama-cpp-router` exists. Ollama (`keep_alive: 0`)
    and llama-swap are the obvious next strategies; a generic configured webhook
    was rejected because it can't express list-then-unload-each, but it would
    suit a backend whose release really is one call.
  - _The reload isn't surfaced._ The eviction says "Freeing GPU memory…", but
    the cold load that follows happens inside the upstream call, where there's
    no signal to read — so a 26B coming back still looks like a slow first
    token.
  - _Thrash._ Alternating chat and image on one GPU pays a full model load each
    way. Inherent to the hardware, but a group that's being switched rapidly
    might warrant a hysteresis window (hold the slot briefly for the same

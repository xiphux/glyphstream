...
- **Model avatars — a face on the bubble, and the persona that draws its own.**
  The UI has settled on messaging-app bubbles with the model's name above each
  reply, so there's already a per-reply identity line with nothing but text on
  it. Giving a custom model an avatar makes a preset read as a _someone_ — which
  is what a preset roleplaying a character already is. Two phases, the second
  much more speculative than the first.

  - _Phase 1 — a static avatar on a custom model._ `custom_models` gains an
    `avatar_media_id`; the bubble's label row renders a small round thumbnail
    beside the name, and the same avatar shows in the model picker, the preset
    list, and the sidebar's conversation rows. Small feature, three constraints
    that decide the shape:
    - **The label is per-message truth, not per-conversation.**
      `assistantLabelForMessage` (`chat-render.ts`) deliberately relabels a
      message whose `modelUsed` differs from the conversation's model — a
      per-turn override or a kept fan-out sibling reads as the model that
      actually produced it. An avatar resolved independently of that would put
      the persona's face on a sibling generated by some other model. So this
      extends that function to return a `{label, avatar}` pair; it does not add
      a second lookup path.
    - **Media lifetime doesn't know about references that aren't
      `message_media`.** The purger reaps `origin='uploaded'` rows whose ref
      count is zero after a 30-minute grace period, so an uploaded avatar
      attached to nothing but a preset row is swept half an hour later. The
      reference has to be visible to `findPurgeCandidates` /
      `stampOrphanedZeroRefRows` (a distinct origin, or a check for FK
      referrers) _before_ the upload path ships — and the gallery-delete and
      "also delete media" conversation-delete paths need the same awareness, or
      they leave a preset pointing at a hard-deleted row.
    - **It's on every bubble, so it must be cheap and it must never reach the
      upstream.** Serve the square thumbnail variant (`getOrCreateThumbnail`)
      with a long cache, not the full asset; and the avatar is presentational
      only — it never enters the request payload, where it would be rent
      charged per turn for a decoration. Writes validate the media row is owned
      by the requesting user (the isolation invariant).

  - _Phase 2 — the conversation designs its own face._ A roleplay preset often
    invents a persona _within_ the conversation that the preset's own avatar
    doesn't depict. So: the model writes an appearance description (features,
    hair, build, dress — whatever the role settled into), that goes to an
    image model, and the result becomes this conversation's avatar, overriding
    the preset's. `conversations.avatar_media_id` is the override, so one
    preset can wear a different face per chat. Where the description comes
    from is the real fork, and the two options have very different costs:
    - _User-initiated (the recommended default)._ A "generate an avatar for
      this character" action in the chat header runs a `memory_model`-class
      pass over the branch to write the appearance description, then
      generates. Zero payload cost, and the user is already the one who
      decides a face is wanted.
    ...
    existing `image_enhancement` enhancer to rewrite the appearance
    description into that model's preferred `prompt_style`, and
    `persistGeneratedImage` so the result is an ordinary generated media row —
    gallery-visible, kept indefinitely, user-deletable.

    Open questions: a "set as avatar" action from the lightbox / gallery is
    probably the cheapest first slice of all of this (it gives both phases a
    manual path and makes fan-out variants a free avatar picker — generate
    four, keep one); whether a persona that changes mid-roleplay
    (transformation, costume change) regenerates in place or wants a
    per-message avatar snapshot ("which face was it wearing at message 40" is
    a real question in a long thread, but a media ref per assistant row is a
    lot of schema for it); whether to store the appearance _description_ (a
    `persona_appearance` column) so regeneration doesn't re-derive it from the
    branch each time; and framing/crop consistency across regenerations.
    ...
    **multimodal pipeline** bet_ — both want a saved (reference image + text
    ...
    (Scope kept deliberately out: user avatars. The user bubble has a label
    too, and GitHub OAuth hands us an avatar URL for free, but that's an
...
- **Virtualized message list.** Long conversations eventually overwhelm the DOM.
  The chat list is a flat, non-virtualized `{#each}` over the whole active
  branch, so a long code-heavy thread piles up layout/paint work (server
  `content_html` is 5-20x the source for shiki blocks).

  **`content-visibility: auto` was tried and REVERTED — don't re-try it without
  first fixing pin-to-bottom.** It looks like the ideal cheap tier (skip
  layout+paint off-screen, keep every node in the DOM, `getElementById` and
  Ctrl-F keep working), but it is incompatible with how this page scrolls:
  off-screen rows collapse to their `contain-intrinsic-size` placeholder, so the
  scroll container's `scrollHeight` stops being the real content height. On a
  40-turn seeded thread the container measured **20638px with it vs 42537px
  without** — a >2x under-report. `scrollToBottom()` is
  `el.scrollTop = el.scrollHeight`, so it scrolls to the bottom of a document
  that doesn't exist; and on stream-finalize the freshly-mounted persisted row
  is briefly collapsed to the placeholder, so the same line snaps the view up
  off the reply that just streamed. Any retry has to make the pin-to-bottom path
  independent of estimated height (e.g. `scrollIntoView` on the last row / the
  bottom sentinel, which forces a skipped row to render) _before_ the CSS goes
...
- **Gallery search & large-library navigation** (asset: permanently-accumulating
  ...
  - _Style / prompt library (auto-mined)._ Mine the user's own generation history
    for recurring prompt patterns and surface them as auto-discovered "your
    styles" — distinct from hand-saved custom-model presets, and from prompt
    _search_ (this clusters for reuse rather than retrieves). **Prompt snippets
    shipped**, which reduces this to a _discovery_ feature rather than a library
    one: the storing, organizing, kind-filtering, and compose-at-the-caret
    mechanics all exist (`prompt_snippets` + the `;` autocomplete), so the
    remaining work is only the clustering pass plus a "save this as a snippet"
    affordance feeding `POST /api/user/prompt-snippets`. Cheap; mostly a query +
    clustering over the prompt history the FTS work already indexes. Open
    question: auto-cluster vs. just a "reuse this prompt" affordance on a gallery
    item, which captures most of the value for far less.
...
- **Re-serializing the branch on every tool-loop iteration.** `rebuildRequestBody`
  ...
  `content_json`, and re-serializes the whole thing once per iteration. The branch
...
- **Unbounded settings payloads.** `/settings/memories` SSRs every memory body plus
  ...
  one is **deliberate, not an oversight**: drill and search rows open the lightbox
  instantly precisely because they already carry the full row, and trimming them
